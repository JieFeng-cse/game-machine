...
def train_seperate(e, train_loader, device, agent1, agent2, optimizer1, optimizer2):
    agent1.train()
    agent2.train()
    correct_pred = 0
    total_num = 0
    ...
    for images, labels in train_loader:
        ...
        v1 = torch.sum(one_hot*log_p1,dim=1).unsqueeze(1)
        v2 = torch.sum(one_hot*log_p2,dim=1).unsqueeze(1)
        tmp = torch.cat([v1,bid1,v2,bid2,labels.unsqueeze(-1)],dim=1)
        log.append(tmp.detach().cpu().numpy())

        total_loss += loss1.item()+loss2.item()
        probs = probs1.detach()
        pred = pred1*probs[:,1] + pred2*probs[:,0]
...
def eval_seperate(eval_loader, device, agent1, agent2):
    agent1.eval()
    agent2.eval()
    correct_pred = 0
    total_num = 0
    ...
    for images, labels in eval_loader:
        ...
        log_p2 = (torch.log(pred2+math.exp(-70)))
        one_hot = torch.zeros(pred2.shape,dtype=float,device='cuda:0').scatter_(1, torch.unsqueeze(labels, dim=1),1)
        loss2 = -torch.sum(one_hot*log_p2,dim=1).unsqueeze(1)*probs2[:,0] + bid2.detach()*probs2[:,1]
        loss2 = loss2.sum()
        loss2/=pred2.shape[0]
...
def main(epochs, seed):
    ...
    new_train_loader = torch.utils.data.DataLoader(new_train_ds, batch_size=128,
                                                shuffle=True, pin_memory=True, num_workers=8)

    validate_loader = torch.utils.data.DataLoader(validate_ds, batch_size=128,
                                                shuffle=True, pin_memory=True, num_workers=8)
    ...
    for e in range(epochs):
        ...
        train_loss, train_acc = train_seperate(e, new_train_loader, device, agent1, agent2, optimizer1, optimizer2)
        val_loss, val_acc = eval_seperate(validate_loader, device, agent1, agent2)
        ...
        if val_acc > best_acc:
            ...
            if not os.path.exists(model_pth):
                os.mkdir(model_pth)
            model1_pth = os.path.join(model_pth, f'50cnnmodel1t{SEED}.pt')
            model2_pth = os.path.join(model_pth, f'50cnnmodel2t{SEED}.pt')
...
def test2():
    ...
    device = torch.device('cuda:0')
    train_ds = MyDataset(train_path)
    pth1=f'model/50cnnmodel1t{SEED}.pt'
    pth2=f'model/50cnnmodel2t{SEED}.pt'
...
def test_write(eval_loader, device, agent1, agent2):
    ...
    print(accuracy, SEED)
    log = np.vstack(log)
    np.savetxt(f'50cnn_log{SEED}.txt',log, fmt='%1.4e')
