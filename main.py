...
def main(epochs, seed):
    ...
    new_train_loader = torch.utils.data.DataLoader(train_ds, batch_size=256,
                                                shuffle=True, pin_memory=True, num_workers=8)

    validate_loader = torch.utils.data.DataLoader(val_ds, batch_size=256,
                                                shuffle=True, pin_memory=True, num_workers=8)
    ...
    # pth1=f'model/cnnmodel1t27_11299.pt'
    # pth2=f'model/cnnmodel2t27_11299.pt'
        ...
        if not (e+1)%500 or val_acc>best_acc:
            if val_acc>best_acc:
                ...
                if not os.path.exists(model_pth):
                    os.mkdir(model_pth)
                model1_pth = os.path.join(model_pth, f'cnnmodel1t{SEED}_sto_na_best.pt') #nl means no penalty loss
                model2_pth = os.path.join(model_pth, f'cnnmodel2t{SEED}_sto_na_best.pt')
                ...
                if not os.path.exists(model_pth):
                    os.mkdir(model_pth)
                model1_pth = os.path.join(model_pth, f'cnnmodel1t{SEED}_sto_na_{e}.pt') #nl means no penalty loss
                model2_pth = os.path.join(model_pth, f'cnnmodel2t{SEED}_sto_na_{e}.pt')
                torch.save(agent1,model1_pth)
                torch.save(agent2,model2_pth)
    print(best_acc)
...
def test2(e_num):
    ...
    pth1 = f'model/cnnmodel1t27_sto_na_{e_num}.pt'
    pth2 = f'model/cnnmodel2t27_sto_na_{e_num}.pt'
    # pth1 = f'model/cnnmodel1t27_sto_na_best.pt'
    # pth2 = f'model/cnnmodel2t27_sto_na_best.pt'
    # pth1='model/model1.pt'
    # pth2='model/model2.pt'
    test_agent1=torch.load(pth1)
    test_agent2=torch.load(pth2)
